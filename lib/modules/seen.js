...
ModuleSeen.prototype.initialize = function ( bot ) {
  ...
  client.addListener('quit', function ( nick, reason, channels, message ) {
    ...
    for (var i = 0, len = botChannels.length; i < len; i++) {
      if (_.indexOf(channels, botChannels[i]) !== -1) {
        buildSeenUser(botChannels[i], nick, that, SeenUser, 'quit')
      }
    }
  })
  ...
  client.addListener('kill', function ( nick, reason, channels, message ) {
    ...
    for (var i = 0, len = botChannels.length; i < len; i++) {
      if (_.indexOf(channels, botChannels[i]) !== -1) {
        buildSeenUser(botChannels[i], nick, that, SeenUser, 'kill')
      }
    }
  })
  ...
  client.addListener('nick', function ( oldnick, newnick, channels, message )  {
    ...
    for (var i = 0, len = botChannels.length; i < len; i++) {
      if (_.indexOf(channels, botChannels[i]) !== -1) {
        buildSeenUser(botChannels[i], oldnick, that, SeenUser, 'nick')
        buildSeenUser(botChannels[i], newnick, that, SeenUser, 'nick')
      }
    }
  })
}
